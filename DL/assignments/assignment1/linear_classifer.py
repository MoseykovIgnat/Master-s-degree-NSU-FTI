...
def cross_entropy_loss(probs, target_index):
    ...
    loss = np.mean(-np.log(probs[np.arange(target_index.shape[0]), target_index.reshape(1,-1)]))
    return loss
...
def softmax_with_cross_entropy(predictions, target_index):
    ...
    # Your final implementation shouldn't have any loops
    onedim = predictions.ndim == 1
    if onedim:
        predictions = predictions.reshape(1,-1)
        target_index = np.array([target_index])  
    loss = cross_entropy_loss(softmax(predictions), target_index)
    target = np.zeros(predictions.shape)
    target[np.arange(predictions.shape[0]), target_index.reshape(1,-1)] = 1
    dprediction = (softmax(predictions) - target) / predictions.shape[0]
    if onedim:
    ...
    def fit(self, X, y, batch_size=100, learning_rate=1e-7, reg=1e-5,
        ...
        for epoch in range(epochs):
            ...
            for batch_indices in batches_indices:
                ce_loss, ce_dW = linear_softmax(X[batch_indices], self.W, y[batch_indices])
